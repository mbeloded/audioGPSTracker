...
public class Accelerometer implements SensorEventListener, Constants {
	...
	public void onSensorChanged(SensorEvent se) {
		...
		if (se.sensor.getType() == accSensor) {
			...
			// deltaX = lastX;//Math.abs(lastX -
			// se.values[SensorManager.DATA_X]);
			// deltaY = lastY;//Math.abs(lastY -
			// se.values[SensorManager.DATA_Y]);
			// deltaZ = lastZ;//Math.abs(lastZ -
			...
			gravity[SensorManager.DATA_X] = alpha
					* gravity[SensorManager.DATA_X] + (1 - alpha) * lastX;
			gravity[SensorManager.DATA_Y] = alpha
					* gravity[SensorManager.DATA_Y] + (1 - alpha) * lastY;
			gravity[SensorManager.DATA_Z] = alpha
					* gravity[SensorManager.DATA_Z] + (1 - alpha) * lastZ;

			linear_acceleration[SensorManager.DATA_X] = lastX
					- gravity[SensorManager.DATA_X];
			linear_acceleration[SensorManager.DATA_Y] = lastY
					- gravity[SensorManager.DATA_Y];
			linear_acceleration[SensorManager.DATA_Z] = lastZ
					- gravity[SensorManager.DATA_Z];
	...
	public void displayCurrentValues() {
		...
		float[] accel = MathUtils.acceleration(linear_acceleration[0],
				linear_acceleration[1], linear_acceleration[2], interval);

		String value_string = "x=" + Float.toString(accel[0]) + " | " + "y="
	...
	public void displayMaxValues() {
		...
		displayText("x=" + Float.toString(deltaXMax) + " | " + "y="
				+ Float.toString(deltaYMax));
		// if (deltaZ > deltaZMax) {
		// deltaZMax = deltaZ;
